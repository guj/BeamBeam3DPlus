...
103 format(4(1x,es15.7))
  ...
  subroutine run_AccSimulator()
    ...
    if(flagGroup.eq.2) then !//Strong-strong using 2 group!!!!!!!!!!!!!!!!!!!!!!!!!!!! 
       ...
       call adios2_io_init(mpicommwd, ierr)       
       call adios2_setup_ptl(nbunch, sumOfNplocals, listOfNplocals(myid+1), ptOut, ptRate, Nturn, ierr)       
       call adios2_setup_phaseout(nbunch, sumOfNplocals*ptFrac, listOfNplocals(myid+1)*ptFrac, Nplocal, ierr)
       ...
       do idbunch = 1, nbunch
          ...
          if(ptOut>0) call ptclout_Output_adios2(Bpts,myid,0,idbunch,ptOut,ptSampleInd)
...
500             continue
          ...
          if((timepass.gt.tmax).and.(saveAll.eq.1)) then
             do idbunch = 1, nbunch
                ...
                call phaseoutBB_adios2(Bpts, Nplocal, i, idbunch, itic, icount,close2g)
             enddo
             saveAll = 0
          endif
          if (mod(i,ptRate).eq.0) then
