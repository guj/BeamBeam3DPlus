...
103 format(4(1x,es15.7))
  ...
  subroutine run_AccSimulator()
    ...
    integer, allocatable, dimension(:) :: listOfNpLocals ! for adios2
    integer :: sumOfNplocals,  temp ! for adios2
    integer*8 :: i8start, i8count; ! for sensei
    ...
    if(flagGroup.eq.2) then !//Strong-strong using 2 group!!!!!!!!!!!!!!!!!!!!!!!!!!!! 
       ...
       call MPI_COMM_SIZE(mpicommwd,comm_size,ierr)
       !write(*,*) myidy, myid, Nplocal, comm_size
       allocate (listOfNplocals(comm_size))
       !call MPI_ALLGATHER(Nplocal,1,MPI_INTEGER, listOfNplocals, 1, MPI_INTEGER, mpicommwd,ierr)
       call MPI_ALLGATHER(ptOut,1,MPI_INTEGER, listOfNplocals, 1, MPI_INTEGER, mpicommwd,ierr)
            

       !write(*,*) listOfNplocals(1), listOfNplocals(2),  listOfNplocals(3), listOfNplocals(4) 
       sumOfNplocals = listOfNplocals(1);
       do i=2, comm_size
          sumOfNplocals = sumOfNplocals + listOfNplocals(i) 
          !listOfNplocals(i) = listOfNplocals(i-1) + listOfNplocals(i) 
       enddo

       do i=comm_size, 2, -1
          temp = 0;
          do j=1,i-1
             temp = temp + listOfNplocals(j);
          enddo
          listOfNplocals(i) = temp;
       enddo
       listOfNplocals(1) = 0;
       

       call sensei_w_init(senseiW, mpicommwd, char(0), ierr)

       !call sensei_io_init(mpicommwd, ierr)       
       !call sensei_setup_ptl(nbunch, sumOfNplocals, listOfNplocals(myid+1), ptOut, ptRate, Nturn, ierr)       
       i8start = listOfNplocals(myid+1)
       i8count = ptOut
       !call sensei_setup_ptl(nbunch, i8start, i8count, ptRate, Nturn, ierr)
       ...
       call sensei_w_turn_start(senseiW, iend, ierr)
       ...
       do idbunch = 1, nbunch
          ...
          if(ptOut>0) call ptclout_Output_sensei(senseiW, Bpts,myid,0,idbunch,ptOut,ptSampleInd)
...
!-----------------------------------------------------------------------------------
       do i = iend + 1, Nturn
          !call sensei_turn_start(ierr, i)
          call sensei_w_turn_start(senseiW, i, ierr)
...
500             continue
          ...
          do idbunch = 1, nbunch
             ...
             if(ptOut>0) then !! save at all turns
                ...
                call ptclout_Output_sensei(senseiW, Bpts,myid,i,idbunch,ptOut,ptSampleInd)
             endif
          ...
          if((timepass.gt.tmax).and.(saveAll.eq.1)) then
             do idbunch = 1, nbunch
                ...
                call phaseoutBB_sensei(senseiW, Bpts, Nplocal, i, idbunch, itic, icount,close2g)
             enddo
             saveAll = 0
          endif
          !call sensei_turn_end(ierr)
