...
100 format(6(1x,es14.7))
  ...
  subroutine phaseoutBB_sensei(senseiW, Bpts, Nplocal, iturn, idbunch, itic, iseedinit,close2g)
    implicit none
    include 'mpif.h'
    type(SenseiBeamWriter), intent(in) :: senseiW
    integer, intent(in) :: iturn,Nplocal,itic, idbunch
    integer*8, intent(in) :: iseedinit
    double precision, dimension(:,:) :: Bpts
    double precision, dimension(2) :: close2g
  ...
  end subroutine phaseoutBB_sensei

  subroutine ptclout_Output_sensei(senseiW, Bpts,myid,iturn,idbunch,npOut,sampleInd)    
    implicit none
    include 'mpif.h'
    type(SenseiBeamWriter), intent(in) :: senseiW
    integer, intent(in) :: myid,iturn,idbunch,npOut
    double precision, dimension(:,:) :: Bpts
    ...
    temp(:) = tmppts(1,:)
    !call sensei_writeAttr(idbunch-1, attrName1, temp);
    call sensei_writeAttr(senseiW, idbunch-1, attrName1, temp, ierr);

    temp(:) = tmppts(2,:)
    !call sensei_writeAttr(idbunch-1, attrName2, temp);
    call sensei_writeAttr(senseiW, idbunch-1, attrName2, temp, ierr);

    temp(:) = tmppts(3,:)
    call sensei_writeAttr(senseiW, idbunch-1, attrName3, temp, ierr);

    temp(:) = tmppts(4,:)
    call sensei_writeAttr(senseiW, idbunch-1, attrName4, temp, ierr);

    temp(:) = tmppts(5,:)
    call sensei_writeAttr(senseiW, idbunch-1, attrName5, temp, ierr);

    temp(:) = tmppts(6,:)
    call sensei_writeAttr(senseiW, idbunch-1, attrName6, temp, ierr);

    deallocate(temp)
    deallocate(tmppts)
  end subroutine ptclout_Output_sensei


  !output the particle information for restart function
  !note::: Nplocal is not used here
  subroutine ptclout_Output(Bpts,myid,iturn,idbunch,Nplocal,npOut,nturns,outRate,sampleInd)
    ...
    if (myid == 0) then
       if (iturn == 0) then 
       write(1234, *)nturns, outRate, npOut
       endif
       write(1234, *)iturn
       write(1234, *)tmppts
    endif
